<?php
/* lxastrocast_v1.php Server-Communication Script for LTrax via ASTROCAST Leo Satellites. Details: see docu
* This is a Webhook
* (C) 18.01.2023 - V1.00 joembedded@gmail.com  - JoEmbedded.de
*
* ASTROCAST has 'A000..' as MAC-Trailer!
*
* Setup in ASTROCAST GUI:
* (named 'Push-API in Astrocast-Docu: https://docs.astrocast.com/docs/api-push )
* Requires: Extra Header 'mailto: MAILADDRESS'
* INFO: Naming Difference: SWARM: 'deviceId' / ASTROCAST: 'deviceGuid'
* ASTROCAST does not need LoggerTime (in metadata), but has a PackedID
*
* 1. URL: https://joembedded.de/ltx/sw/lxastrocast_v1.php
* 2. Headers:   apikey D_API_KEY!
* Will allow to Auto-generate device!
*
* if $dbg>0: Allow simulated Payload, e.g.
*  http://localhost/ltx/sw/lxastrocast_v1.php?x={"messageGuid":"d72743ce-ea89-4eb1-98c9-1d51f7c3128f","deviceGuid":"47156563-31af-4487-8084-24145baef07c","deviceGroupGuid":"85bcb78f-216a-4423-9363-80b01c4ae465","createdDate":"2023-01-11T07:00:19","receivedDate":"2023-01-11T10:30:34.5700000","latitude":null,"longitude":null,"data":"gQACALIB/QAAAloW4VsDnQ==","messageSize":16}
*
* *todo* 
* - Linearise/Alarms
*/
...
function decode_payloadRTN($strdata) // Typ 129!
{
	...
	if ($cnt - $idx >= 2) { // 129: packetID and Traveltime
		$pidx = get_u16(substr($strdata, $idx, 2));
		$idx += 2;

		// Different to SWARM
		$tzo = timezone_open('UTC');
		$cstr=$args['createdDate'];
		$rstr=$args['receivedDate'];

		$unixsecs = date_create($cstr,$tzo)->getTimestamp();
		$rec_ts = date_create($rstr,$tzo)->getTimestamp();
		$devi['dtime'] = $unixsecs;
		//$txtdata.= sprintf(' Time(UTC):%s', gmdate("d.m.Y H:i:s",$unixsecs));
		$dline = "!$unixsecs";
		$uline = "!U";
		// Generate Traveltime:
		$traveltime = $rec_ts - $unixsecs;
	}
	if ($cnt - $idx >= 1) {
...
function add_data($cont)
{
	...
	if (@filesize($dpath . "/files/data.edt") > 1000000) {	// 2*1MB pro ASTROCAST
...
function check_iparam()
{
	...
	if ($ikanist < 1 || $idxk0 < 0 || !isset($iparam) || count($iparam) < 19) {
		...
		$iparam = array("@100", $devi['typ'], 44, '3', $now,   "AMA" . substr($mac, -11), '3600', '0', '0', '0', '0', '3600', '1', '0', '1', '1', '0', '-40', '0');
		$ikanist = 0;
	}
...
$data = base64_decode($args['data']); // decode Payload, ASTROCAST: max 160 U.8
$maxlen = strlen($data);
if ($maxlen < 1) exit_error("Empty Data");	// 16 is minimum

if (!isset($args['deviceGuid'])) exit_error("No JSON 'deviceGuid'");

$mac =  'A' . str_pad(strtoupper(dechex(substr($args['deviceGuid'],0,8))), 15, '0', STR_PAD_LEFT);	// ASTROCAST exactly 'A'+15 Zeichen!
if (strlen($mac) != 16) {
	exit_error("MAC Len");
}
...
if (check_dirs(false)) exit_error("Error (Directory/MAC not found)"); // May write some ASTROCAST unnecary files
...
for (;;) {
	...
	$cmd = ord($data[$idx]);
	switch ($cmd) {
		case 129:  // 129: Kennung RTN-Block ASTROCAST
			$idx++;
...
$xlog = "($maxlen Bytes Data, Packet:$pidx)"; // Set from decode()
$opidx = intval(@$devi['pidx']); // Index of last packet
if ($pidx != (($opidx + 1) % 65000)) {	// SWARM/ASTROCAST: 0..64999
