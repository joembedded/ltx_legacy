...
// Default-iparam generieren - *todo*: Set action/Units/Digits
function gen_default_iparam($mac, $dname)
{
	global $now, $xlog;
	$typ = 930; // Aquatos LoRa
	$prefix = "ALR";

	$iname = $prefix . substr($mac, -11);
	if (strlen($dname)) $iname .= ' ' . $dname;
	$iparam = array("@100", $typ, 48 /*channels*/, '0' /*HK_FLAGS*/, $now, $iname, '60' /* Period Unknown*/, '0', '0', '0', '0', '0', '1', '0', '1', '1', '0', '-40', '0', '');
...
try {
	...
	// Get the body of the POST request OR use test=fname (from develop directory)
	if (isset($_GET['test'])) {
		$ftname = $_GET['test'];
		$xlog .= "(Test:'$ftname')";
		$payload = file_get_contents("./develop/lora/$ftname.json");
		if (isset($_GET['show'])) {	// show: Show payload and exit
			echo $payload;
			exit;
		}
	} else {
	...
	$dname = $data['deviceInfo']['deviceName'] ?? $data['end_device_ids']['device_id'] ?? '';
	...
	$lockf = @fopen("$dpath/date0.dat", 'r');
	if (!$lockf) $xlog .= "(Error: Open 'date0.dat')";
	else while (!flock($lockf, LOCK_EX)) usleep(10000);  // Lock File - Is a MUST		

	// For debug: blackbox-payloads
	if ($dbg) {
		if (@filesize("$dpath/dbg/indata.log") > 100000) {	// Main LOG
			@unlink("$dpath/dbg/indata.log/_indata_old.log");
			rename("$dpath/dbg/indata.log", "$dpath/dbg/_indata_old.log");
		}
		file_put_contents("$dpath/dbg/indata.log", $now_str . ":\n" . $payload . "\n\n", FILE_APPEND);
	}

	// Shift new Parameters?
	$wiparam = false;	// WriteIparam
	...
	$fport = $data['fPort'] ?? $data['uplink_message']['f_port'] ?? '-199';
	...
	// ----- LTX Payload START -----
	if ($object !== null && $maxlen > 0) {
		// Extract known units to array
		$known_units = [];
		// Param/Data-Files
		$idxkan0 = 0;	// if >= 19 Index of Kan0
		$iparam = @file("$dpath/files/iparam.lxp", FILE_IGNORE_NEW_LINES);
		if ($iparam === false) {
			$iparam = gen_default_iparam($mac, $dname);
			$wiparam = true;
		}
		$il = count($iparam);
		for ($i = 19; $i < $il; $i++) { // Jeder Kanal muss 14 Zeilen exakt haben
			if (@$iparam[$i][0] == '@') {
		...
		foreach ($ochana as $chan) {
			...
			if ($idx < 90 && $highestchan < $idx) $highestchan = $idx;
			$unit = @$chan['unit'];
			$val = @$chan['value'] ?? '?';
		...
		if ($wiparam) {
			...
			for ($i = 0; $i <= $highestchan; $i++) {
				$bidx = $idxkan0 + $i * 14;
				$iparam[$bidx] = "@$i";	// Das ist immer fix
				if (!isset($iparam[$bidx + 1])) $iparam[$bidx + 1] = '1'; //action
				...
				if (!isset($iparam[$bidx + 9])) $iparam[$bidx + 9] = '1.0';
				if (!isset($iparam[$bidx + 10])) $iparam[$bidx + 10] = '0.0';
				if (!isset($iparam[$bidx + 11])) $iparam[$bidx + 11] = '0.0';
				if (!isset($iparam[$bidx + 12])) $iparam[$bidx + 12] = '0';
	...
	} else { // No Payload? = On ChirpStack different reasons: JOIN, TIME, ERRORS, ...
		$reason = 128;	// Unknown
		if (isset($data['batteryLevel']) || isset($data['margin'])) {
			$edtdata = array("<BAT ($timestr)>\n");
		} else if (isset($data['level'])) {
			$edtdata = array("<'" . $data['level'] . "':'" . @$data['description'] . "' ($timestr)>\n");
		} else if (isset($data['devAddr']) && !isset($data['data'])) {
			$joincnt = @$devi['joincnt'] ?? 0; // Count each JOIN
			$edtdata = array("<JOIN $joincnt:'" . $data['devAddr'] . "' ($timestr)>\n");
			$devi['joincnt'] = $joincnt + 1;
		} else {
