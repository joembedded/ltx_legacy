<?PHP
	...
	include("../../sw/conf/api_key.inc.php");
     ...
     function error_message($e){  // Die with Error
echo <<<ERR
...
?>
  ...
  function wmove(typ){
    ...
    dy3=(maxy-miny)/5;   //20%
    if(typ==0){    // Anfang
      minx=o_minx; maxx=minx+dx;
    }else if(typ==1){ // Ende
      minx=o_maxx-dx; maxx=o_maxx;
    }else if(typ==2){ // Rueck
  ...
  function mousemove(e) {
    ...
    if(mx>sizex-cx1) mx=sizex-cx1;    else if(mx<cx0) mx=cx0;
    if(my>sizey-cy1) my=sizey-cy1;    else if(my<cy0) my=cy0;
