...
<?php
	...
	$plat=-1; $plng=-1; // Inices der Koordianten
	$time=time();
    foreach($data as $line){
        ...
        if($line[1]=='U'){ 
			...
			if($dbg){ echo "DBG: Index plat/plng:$plat/$plng\n"; }
		}else{
...
?>
	...
	var polyline = L.polyline(gps_points, {color: 'red', opacity: 0.3}).addTo(mymap);

	if(gps_points.length>0){
		var markerx = L.marker(gps_points[gps_points.length-1], {opacity: 0.9}).addTo(mymap);
		var marker = L.marker(gps_points[gps_points.length-1], {opacity: 0.5}).addTo(mymap);
