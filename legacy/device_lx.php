...
<?php
	...
	case 2:	echo "AUTO"; break;
	case 3: echo "MANUAL"; break;
	case 4: echo "SMS"; break;
	default: echo "UNKNOWN(reason=$reason)"; break;	// Alarm e.g. t.b.d
	}
	...
	if(!$demo){
		...
		echo "Device Firmware "; if(!empty($devi['fw_ver'])) echo "Version: V".($devi['fw_ver']/10); else echo "(unknown)";
		$fwt=@$devi['fw_cookie'];
		...
		echo "<br>";
		echo "Device Type: "; if(!empty($devi['typ'])) echo $devi['typ']."<br>"; else echo"(unknown)<br>";
		echo "SIM (IMSI): "; if(!empty($devi['imsi'])) echo "'".$devi['imsi']."'<br>"; else echo"(unknown)<br>"; 
	...
	if(!$demo){
		...
		if($ds>0){
			...
			$fa=secs2period($dt);
			echo "<a href=\"view.php?s=$mac&f=conn_log.txt\">Connection Logfile 'conn_log.txt'</a> ($ds Bytes, Age: $fa)<br>";
		}else{
		...
		if($ds>0){
			...
			$fa=secs2period($dt);
			echo "<a href=\"view.php?s=$mac&f=_conn_log_old.txt\">Old Connection Logfile '_conn_log_old.txt'</a> ($ds Bytes, Age: $fa)<br>";
		}else{
